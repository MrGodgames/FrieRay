...
static LAST_STATS: std::sync::OnceLock<tokio::sync::Mutex<(u64, u64, std::time::Instant)>> =
...
pub async fn get_traffic_stats(state: State<'_, AppState>) -> Result<TrafficStats, String> {
    ...
    let uplink = query_stat(&xray_bin, "outbound>>>proxy>>>traffic>>>uplink")
        .await
        .unwrap_or(0);
    let downlink = query_stat(&xray_bin, "outbound>>>proxy>>>traffic>>>downlink")
        .await
        .unwrap_or(0);
    ...
    let (up_speed, down_speed) = if elapsed > 0.1 && last.0 > 0 {
        ...
        let down_diff = if downlink > last.1 {
            downlink - last.1
        } else {
            0
        };
...
pub async fn speed_test(state: State<'_, AppState>) -> Result<f64, String> {
    ...
    for (name, url) in SPEED_TEST_TARGETS {
        match run_speed_test(&client, url).await {
            Ok((bytes, elapsed, mbps)) => {
                state
                    .logs
                    .add(
                        "success",
                        &format!(
                            "Скорость: {:.1} Мбит/с через {} ({} байт за {:.1}с)",
                            mbps, name, bytes, elapsed
                        ),
                    )
                    .await;
                return Ok(mbps);
    ...
    let message = format!(
        "Тест скорости не удался ни через одно зеркало: {}",
        errors.join(" | ")
    );
    state.logs.add("error", &message).await;
    Err(message)
}
...
async fn run_speed_test(client: &reqwest::Client, url: &str) -> Result<(usize, f64, f64), String> {
