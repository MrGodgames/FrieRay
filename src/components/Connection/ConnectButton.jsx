...
export default function ConnectButton({ connected, connecting, onToggle }) {
    ...
    return (
        <div className="connect-wrapper">
            <div className={`connect-area ${isClassic ? 'classic' : ''}`}>
                {!isClassic && (
                ...
                <button
                    className={`connect-btn ${connected ? 'connected' : ''} ${connecting ? 'connecting' : ''} ${isClassic ? 'classic' : ''}`}
                    onClick={onToggle}
                    disabled={connecting}
                >
                    <div className="connect-core">
                        {connecting ? (
                            ...
                            <span className="connect-rune">{isClassic ? (connected ? '✓' : '⏻') : (connected ? '⟡' : '✦')}</span>
                        )}
                    </div>
                </button>
            </div>

            <span className={`connect-label ${isClassic ? 'classic' : ''}`}>
                {connectLabel}
            </span>
        </div>
    );
